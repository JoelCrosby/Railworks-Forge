...
<div class="mx-auto px-6 py-8">
	<header class="mb-8 flex items-center justify-between">
		...
		<div class="flex items-center gap-2">
			<button
				class="cursor-pointer rounded-md border border-transparent bg-transparent px-2 py-1 text-base leading-none text-muted hover:border-border-strong hover:bg-surface-raised hover:text-text"
	...
	{#if error}
		<div class="mb-6 rounded-md border border-danger-border bg-danger-surface px-4 py-3 text-sm text-danger-text">
	...
	{#if loading}
		<div class="mt-8 text-center text-sm text-muted">{progress ?? t(locale, 'home-scanning-routes')}</div>
	{:else if routes.length === 0 && !error && !gamePathMissing}
		<div class="mt-8 text-center text-sm text-muted">{t(locale, 'home-no-routes')}</div>
	{:else}
				...
				<li>
					...
					>
						...
						{#if route.description}
							<span class="flex-2 truncate text-[0.8rem] text-muted">{route.description}</span>
						{/if}
						<span
