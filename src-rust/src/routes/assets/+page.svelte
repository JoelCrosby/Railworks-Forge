<script lang="ts">
  ...
  $effect(() => {
    setBreadcrumbs([{ label: t(locale, 'nav-assets'), href: '/assets' }]);
  });
...
<div class="mx-auto max-w-275 p-6">
  <header class="mb-6 flex items-start justify-between gap-4">
    <div>
      ...
      {#if !loading && nodes.length > 0}
        <p class="mt-1 text-[0.8rem] text-muted">
  ...
  {#if !loading && nodes.length > 0}
    <div class="mb-4 flex items-center gap-4">
      ...
      />
      <label
        class="flex cursor-pointer items-center gap-1.5 whitespace-nowrap text-[0.8rem] text-muted"
      >
  ...
  {#if loading}
    <div class="mt-8 text-center text-sm text-muted">
      {t(locale, 'assets-scanning')}
    </div>
  {:else if groups.length === 0 && !error}
    <div class="mt-8 text-center text-sm text-muted">
      {nodes.length === 0
        ? t(locale, 'assets-no-assets')
        : t(locale, 'assets-no-matches')}
    </div>
  {:else}
    <div class="flex flex-col gap-2">
      {#each groups as group (group.name)}
        ...
        >
          <summary
            class="flex cursor-pointer list-none items-center gap-2 px-4 py-2.5 text-sm font-semibold select-none [&::-webkit-details-marker]:hidden"
          >
            {group.name}
            <span class="text-[0.8rem] font-normal text-muted"
              >({group.products.length})</span
            >
          </summary>

          <div
            class="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-1.5 p-2"
          >
            {#each group.products as node (node.product)}
              <div
                class="flex items-center justify-between gap-2 rounded-md border border-surface-raised bg-bg px-3 py-2"
              >
