...
<div class="mx-auto max-w-300 p-6">
	<Breadcrumb items={[
		...
		{ label: scenario?.name ?? scenarioId, onclick: backToScenario },
		{ label: consist?.serviceName || consist?.locomotiveName || 'Consist' }
	]} />
	...
	{#if successMsg}
		<div class="mb-4 rounded-md border border-success-border bg-success-surface px-4 py-3 text-sm text-success-text">{successMsg}</div>
	{/if}
	{#if error}
		<div class="mb-4 rounded-md border border-danger-border bg-danger-surface px-4 py-3 text-sm text-danger-text"><strong>Error:</strong> {error}</div>
	{/if}
	...
	{#if consist}
		<section class="mt-2">
			<div class="mb-3 flex items-center gap-4">
				<h2 class="text-base font-semibold">Vehicles <span class="font-normal text-muted">({consist.vehicles.length})</span></h2>
			</div>

			{#if consist.vehicles.length === 0}
				<div class="mt-8 text-center text-sm text-muted">No vehicles in this consist.</div>
			{:else}
				<table class="w-full border-collapse text-[0.8rem]">
					<thead>
						<tr>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">#</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Type</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Name</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Number</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Provider</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Blueprint</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Flip</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">State</th>
							<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted"></th>
						</tr>
					</thead>
					<tbody>
						{#each consist.vehicles as v (v.index)}
							<tr class="hover:bg-surface">
								<td class="w-8 border-t border-border px-2.5 py-1.5 align-middle text-border-strong">{v.index + 1}</td>
								...
								<td class="border-t border-border px-2.5 py-1.5 align-middle font-medium">{v.name || '—'}</td>
								<td class="border-t border-border px-2.5 py-1.5 align-middle text-border-strong">#{v.uniqueNumber}</td>
								<td class="max-w-32 truncate whitespace-nowrap border-t border-border px-2.5 py-1.5 align-middle text-muted">{v.blueprint.provider}</td>
								<td class="max-w-80 truncate whitespace-nowrap border-t border-border px-2.5 py-1.5 align-middle text-border-strong">{v.blueprint.blueprintId}</td>
								<td class="border-t border-border px-2.5 py-1.5 text-center align-middle">{v.flipped ? '↩' : ''}</td>
								<td class="border-t border-border px-2.5 py-1.5 align-middle">
...
{#if showReplaceDialog}
	<div class="fixed inset-0 z-100 flex items-center justify-center bg-black/70" role="dialog" aria-modal="true">
		<div class="flex max-h-[85vh] w-[min(720px,95vw)] flex-col gap-4 overflow-y-auto rounded-[10px] border border-surface-raised bg-surface p-6">
			...
			{#if replaceError}
				<div class="mb-4 rounded-md border border-danger-border bg-danger-surface px-4 py-3 text-sm text-danger-text"><strong>Error:</strong> {replaceError}</div>
			{/if}
			...
			<div class="flex max-h-50 flex-col gap-1 overflow-y-auto rounded-md border border-surface-raised p-2">
				{#if replaceEntries.length === 0}
					<div class="mt-8 text-center text-sm text-muted">Add vehicles below.</div>
				{:else}
					{#each replaceEntries as entry, i}
						<div class="flex items-center gap-2 rounded px-1.5 py-1 text-[0.78rem] hover:bg-bg">
							<span class="w-5 shrink-0 text-right text-border-strong">{i + 1}</span>
							...
							<span class="flex-1 truncate whitespace-nowrap text-border-strong">{entry.blueprintId}</span>
							{#if entry.flipped}<span class="shrink-0 text-muted">↩</span>{/if}
							<button
			...
			<details class="rounded-md border border-surface-raised p-3">
				<summary class="cursor-pointer text-sm text-muted select-none">Add vehicle</summary>
				<div class="mt-3 grid grid-cols-2 gap-x-4 gap-y-2">
					...
					<label class="flex flex-row items-center gap-2 text-[0.78rem] text-text">
