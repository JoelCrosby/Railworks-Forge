...
<div class="mx-auto max-w-300 p-6">
	...
	{:else if tracks.length === 0 && !error}
		<div class="mt-8 text-center text-sm text-muted">No track blueprints found. The route may not have a Tracks.bin.</div>
	{:else}
		<div class="mt-2">
			<div class="mb-3 flex items-center gap-4">
				<h2 class="text-base font-semibold">Track Blueprints <span class="font-normal text-muted">({tracks.length})</span></h2>
				{#if pendingCount() > 0}
					<span class="text-[0.8rem] text-warn">{pendingCount()} pending replacement(s)</span>
				{/if}
			</div>
			...
			<table class="w-full border-collapse text-[0.8rem]">
				<thead>
					<tr>
						...
						<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Product</th>
						<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Blueprint ID</th>
						<th class="border-b border-surface-raised px-2.5 py-1.5 text-left font-medium text-muted">Replace with</th>
				...
				<tbody>
					{#each tracks as track (trackKey(track))}
						...
						<tr class={repl !== null ? 'bg-[#1a2020] hover:bg-[#1a2530]' : 'hover:bg-surface'}>
							<td class="whitespace-nowrap border-t border-border px-2.5 py-1.5 align-middle text-muted">{track.provider}</td>
							<td class="whitespace-nowrap border-t border-border px-2.5 py-1.5 align-middle text-muted-strong">{track.product}</td>
							<td class="max-w-80 truncate border-t border-border px-2.5 py-1.5 align-middle text-xs text-border-strong">{track.blueprintId}</td>
							<td class="max-w-96 border-t border-border px-2.5 py-1.5 align-middle">
								{#if repl}
									<div class="flex items-center gap-1 text-xs">
										<span class="whitespace-nowrap text-ok">{repl.provider}</span>
										<span class="shrink-0 text-surface-raised">›</span>
										<span class="whitespace-nowrap text-success-text">{repl.product}</span>
										<span class="shrink-0 text-surface-raised">›</span>
										<span class="truncate text-success-border">{repl.blueprintId}</span>
									</div>
								{:else}
									<span class="text-surface-raised">—</span>
								{/if}
							</td>
							<td class="flex gap-1 whitespace-nowrap border-t border-border px-2.5 py-1.5 align-middle">
								<button
...
{#if editingKey !== null}
	...
	<div class="fixed inset-0 z-100 flex items-center justify-center bg-black/70" role="dialog" aria-modal="true">
		<div class="flex w-[min(560px,95vw)] flex-col gap-4 rounded-[10px] border border-surface-raised bg-surface p-6">
			...
			{#if sourceTrack}
				<div class="flex items-baseline gap-2 rounded-md border border-surface-raised bg-bg px-3 py-2 text-[0.8rem]">
					<span class="shrink-0 text-muted">Replacing:</span>
					<span class="truncate text-muted-strong">{sourceTrack.provider} / {sourceTrack.product} / {sourceTrack.blueprintId}</span>
				</div>
			{/if}
